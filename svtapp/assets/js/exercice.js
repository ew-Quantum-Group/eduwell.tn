...
/* ══════════════════════════════════════════════════════
██  FREE MODE — GLOBAL FUNCTION (for exercice.js)
══════════════════════════════════════════════════════ */
/** Returns true if Free Mode is currently ON */
function isFreeMode() {
  try {
    return localStorage.getItem('svtapp_free_mode') === 'true';
  } catch(e) {
    return false;
  }
}
...
// 🔥 FREE MODE FIX: Bypass lock check if Free Mode is ON
const isUnlocked = isFreeMode() || !!sData.unlocked;
const isCompleted=!!sData.completed;
...
if(idx >0){
  ...
  path.appendChild(conn); // Fixed: was "appen dChild"
}
...
const allDone=STAGES.every(s=>(STATE.stages[s.id]||{}).completed); // Fixed: was "STAGE S.every"
const tw=document.createElement('div');
tw.className='node-wrap center'; tw.style.animationDelay='.40s';
tw.innerHTML=`<button class="node-btn trophy-btn${allDone?' unlocked':''}" onclick="${allDone?`showCongrats()`: `showToast('fa-lock','Terminez toutes les étapes d\\'abord !')`}"><i class="fas fa-trophy"></i></button><div class="stage-lbl" style="margin-top:9px">Grand Prix</div><div class="node-name">🏆 Tout compléter</div>`;
path.appendChild(tw);
}
...
// 🔥 FREE MODE FIX: Show locked tip only if NOT free mode AND not unlocked
const lt=document.getElementById('shLockedTip');
lt.style.display = (isFreeMode() || sData.unlocked) ? 'none' : 'flex';
...
// Start button — 🔥 FREE MODE FIX: Enable if Free Mode OR unlocked
const startBtn=document.getElementById('shStartBtn');
if(isFreeMode() || sData.unlocked){
...
btn.innerHTML=`<div class="opt-icon-wrap"><div class="opt-icon"><i class="fa-solid ${q.icons?.[i]||'fa-circle'}"></i></div></div><div class="opt-inner"><span class="opt-ltr">${LTR[i]}</span><span class="opt-text">${esc(opt)}</span></div><div class="opt-cb"><i class="fa-solid fa-check"></i></div>`;
...
continueBtn.innerHTML=' Retour à l\'accueil';
...
const d=(1.2+Math.random()*1.5).toFixed(2), dl=(Math.random()*0.4).toFixed(2);
...
{ id: "dc3-c4", name: "Correction n°3 - 4",           type: "correction", pages:1, url: "https://drive.google.com/file/d/1/preview" }
]
},
{
id:  "ds1",
...
document.getElementById('exHeroLabel').innerHTML = ' BAC SVT · 2026';
...
document.getElementById('exSecLabel').innerHTML = ' Cours';
document.getElementById('exSecCount').textContent = total;
grid.innerHTML = COURS_DATA.map((c,i)=> `<div class="ex-card ${c.color}" onclick="exOpenCoursePdf('${c.id}')" style="animation:nodeIn .45s var(--ease) ${i*60}ms both"><div class="ex-card-icon"><i class="fa-solid ${c.icon}"></i></div><div><div class="ex-card-name">${c.name}</div><div class="ex-card-sub">${c.short} · BAC SVT</div></div><div class="ex-card-count"><i class="fa-solid fa-book-open"></i>${c.lessons} leçons</div><div class="ex-card-prog"><div class="ex-card-prog-track"><div class="ex-card-prog-fill" style="width:0%"></div></div><div class="ex-card-prog-label">0% Complete</div></div></div>`).join('');
} else {
...
document.getElementById('exSecLabel').innerHTML = ' Chapitres';
document.getElementById('exSecCount').textContent = EXERCICES_DATA.length;
grid.innerHTML = EXERCICES_DATA.map((cat,i)=> `<div class="ex-card ${cat.color}" onclick="exOpenSheet('${cat.id}')" style="animation:nodeIn .45s var(--ease) ${i*60}ms both"><div class="ex-card-icon"><i class="fa-solid ${cat.icon}"></i></div><div><div class="ex-card-name">${cat.name}</div><div class="ex-card-sub">${cat.short} · BAC SVT</div></div><div class="ex-card-count"><i class="fa-solid fa-file"></i>${cat.docs.length} docs</div><div class="ex-card-prog"><div class="ex-card-prog-track"><div class="ex-card-prog-fill" style="width:0%"></div></div><div class="ex-card-prog-label">0% consulté</div></div></div>`).join('');
}
}
...
list.innerHTML = cat.docs.map(doc=> `<div class="ex-sheet-item ${doc.type==='correction'?'correction':doc.type==='cours'?'cours':''}" onclick="exOpenPdf('${doc.id}')"><div class="ex-sheet-item-icon"><i class="fa-solid ${doc.type==='cours'?'fa-book-open':'fa-file-lines'}"></i></div><div class="ex-sheet-item-info"><div class="ex-sheet-item-name">${doc.name}</div><div class="ex-sheet-item-meta"><i class="fa-regular fa-file-lines"></i> ${doc.pages} page${doc.pages>1?'s':''} &nbsp;·&nbsp; <i class="fa-solid fa-tag"></i> ${doc.type==='correction'?'Correction':doc.type==='cours'?'Cours PDF':'Devoir'}</div></div><i class="fa-solid fa-chevron-right ex-sheet-item-arrow"></i></div>`).join('');
